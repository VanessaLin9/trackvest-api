...
describe('Transactions sell FIFO (e2e)', () => {
  ...
  it('rebuilds sell lot matches and reposts sell GL when an earlier buy is updated', async () => {
    ...
    const buy1 = await request(app.getHttpServer())
      .post('/transactions')
      .set(auth(user.id))
      .send({
    ...
    const activeSellEntries = await prisma.glEntry.findMany({
      where: {
    ...
    const archivedSellEntries = await prisma.glEntry.findMany({
      where: {
        refTxId: sell.body.id,
        isDeleted: true,
      },
